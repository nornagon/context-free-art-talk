...
Renderer = {
	...
	width: null,
	height: null,
	random: new MersenneTwister(1533101576904),
	...
	render: function( compiled, canvas ) {
	...
	tick: function(){
      ...
      var end = new Date();
	    ...
	    Renderer._nextTick = setTimeout( Renderer.tick, 2*(end-start) );
	  }
	  Renderer._rendering = false;
	},
  ...
  stop: function() {
    Renderer.queue = []
    clearTimeout(Renderer._nextTick)
  },
	...
	drawRule: function( ruleName, transform, color, priority ){
		...
		var r = Renderer.random.random() * sum;
...
function contextFree( text, canvas ) {
  var t = new Tokenizer();
